      ...
      <Card className="w-full h-full bg-[#f5e6d3] shadow-xl relative overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center p-6 overflow-y-auto no-scrollbar">
          <motion.h2
            ...
            className="text-4xl font-scrapbook-title text-[#5c4934] mb-6 whitespace-nowrap"
          >
            Skills & Achievements
          </motion.h2>

          <div className="w-full px-4 mb-6">
            <h3 className="text-2xl font-scrapbook-title text-[#5c4934] mb-4 text-center">My Toolkit</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(skills).map(([category, skillList], index) => (
                ...
                >
                  ...
                  >
                    <h4 className="text-lg font-scrapbook-accent text-[#5c4934] mb-2 font-semibold">{category}</h4>
                    <div className="flex flex-wrap gap-2">
                      {skillList.map((skill, i) => (
                        <motion.span
                          key={i}
                          whileHover={{ scale: 1.1, backgroundColor: '#8b7355', color: '#fff9f0' }}
                          className="bg-[#d4b595] px-3 py-1 rounded-full text-[#fff9f0] text-sm whitespace-nowrap cursor-pointer transition-colors duration-200 font-scrapbook-playful"
                        >
                          {skill}
                        </motion.span>
                      ))}
          ...
          <div className="w-full px-4">
            <h3 className="text-2xl font-scrapbook-title text-[#5c4934] mb-4 text-center">Milestones & Accolades</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {achievements?.map((achievement, index) => (
                ...
                >
                  ...
                  >
                    <p className="text-[#5c4934] text-base font-scrapbook-accent font-semibold mb-1">{achievement.title}</p>
                    <p className="text-[#8b7355] text-sm font-scrapbook-casual">{achievement.description}</p>
                  </motion.div>
                </motion.div>
              ))}
            </div>
